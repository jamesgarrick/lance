...
app.post("/auth/login", async (c) => {
  ...
  if (RESERVED_SCOPES.has(scope)) return c.json({ error: `scope reserved: ${scope}` }, 403);
...
app.get("/auth/whoami", async (c) => {
  ...
  const payload = await verifyToken(auth.slice("Bearer ".length)).catch(() => null);
...
app.get("/search", async (c) => {
  ...
  const rows = await db.select().from(packages).where(like(packages.name, `%${q}%`)).limit(50);
  const results = await Promise.all(
    rows.map(async (pkg) => {
      const latest = await db.select().from(versions)
        .where(and(eq(versions.packageId, pkg.id), eq(versions.active, true), isNull(versions.unpublishedAt)))
...
app.get("/packages/:fullName", async (c) => {
  ...
  const pkg = await db.select().from(packages).where(eq(packages.name, fullName)).limit(1);
  if (!pkg[0]) return c.json({ error: "Package not found" }, 404);
  const vers = await db.select().from(versions)
    .where(and(eq(versions.packageId, pkg[0].id), eq(versions.active, true), isNull(versions.unpublishedAt)))
    .orderBy(desc(versions.createdAt));
  return c.json({
    name: pkg[0].name,
...
app.post("/publish", async (c) => {
  ...
  if (!semver.valid(body.version)) return c.json({ error: "Invalid semver version" }, 400);

  const [scope, packageName] = body.name.split("/");
  if (!scope || !packageName) return c.json({ error: "Invalid package name format" }, 400);
  if (scope !== scopeFromUser(payload.user)) {
    return c.json({ error: `Cannot publish outside your scope (${scopeFromUser(payload.user)})` }, 403);
  }

  let pkg = (await db.select().from(packages).where(eq(packages.name, body.name)).limit(1))[0];
  if (!pkg) {
    const inserted = await db.insert(packages).values({ name: body.name, scope, packageName }).returning();
    pkg = inserted[0]!;
  }
  ...
  const existing = await db
    .select()
    .from(versions)
    .where(and(eq(versions.packageId, pkg.id), eq(versions.version, body.version)))
    .limit(1);
  if (existing[0]) return c.json({ error: "Version already published (immutable)" }, 409);
...
app.delete("/packages/:fullName/versions/:version", async (c) => {
  ...
  const pkg = (await db.select().from(packages).where(eq(packages.name, fullName)).limit(1))[0];
  if (!pkg) return c.json({ error: "Package not found" }, 404);
  const scope = fullName.split("/")[0];
  if (scope !== scopeFromUser(payload.user)) return c.json({ error: "Forbidden" }, 403);

  const row = (await db.select().from(versions).where(and(eq(versions.packageId, pkg.id), eq(versions.version, version))).limit(1))[0];
  ...
  if (hours > 72) return c.json({ error: "Unpublish window exceeded (72h)" }, 403);
