(() => {
   ...
   * @param {string} className
   */
  function assignPath(target, path, className) {
    let cursor = target;
    for (let i = 0; i < path.length - 1; i += 1) {
      ...
      if (typeof next !== "object" || next === null || Array.isArray(next)) {
        throw new Error(`Path collision while assigning ${className} at "${path.join(".")}"`);
      }
      cursor = /** @type {Record<string, unknown>} */ (next);
    }

    cursor[path[path.length - 1]] = className;
  }
  ...
  for (const row of rows) {
    ...
    assignPath(cfgData, [prefix, family, variant], className);
  }
