...
export type CfgNodeBase<T> = T extends {
	readonly __cfgBase: infer TBase extends string;
}
	? TBase
	: Extract<T, string>;

export type CfgNodeChildren<T> =
	T extends CfgNode<string, infer TChildren extends object>
		? TChildren
		: T extends object
			? T
			: never;
...
export function cfgNode<
	...
	const TChildren extends object,
>(base: TBase, children: TChildren): CfgNode<TBase, TChildren> {
...
export class ConfigFile {
	...
	with(...contributions: readonly CfgContribution[]): ConfigFile {
		return new ConfigFile({
			cfgWeapons: mergeCfgTrees(
				this.cfgWeapons,
				...contributions.map((c) => c.cfgWeapons).filter(isCfgTree),
			),
...
export function mergeCfgTrees(
	base: CfgTree,
	...overlays: readonly CfgTree[]
): CfgTree {
