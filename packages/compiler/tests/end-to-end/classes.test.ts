...
describe("class lowering to hashmap model", () => {
    ...
    `);
		...
		expect(sqf).toContain(
			'private _car = ["Humvee", 0] call LANCE_Vehicle_new;',
		);
    ...
    `);
		...
		expect(functionBodies).toContain(
			'_self set ["_types", ["M1_Abrams", "Tank", "Vehicle"]]',
		);
		expect(sqf).toContain(
			'private _isVehicle = _tank get "_types" find "Vehicle" != -1;',
		);
		expect(diagnostics.toArray()).toEqual([]);
	});
});
