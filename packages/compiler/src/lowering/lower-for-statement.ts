...
export function lowerForStatement(
	...
	return {
		...
		to: toExpr,
		step:
			shape.direction === "down" ? { kind: "Literal", text: "-1" } : undefined,
 ...
 */
function recognizeNumericFor(
	statement: ForStatement,
): NumericForShape | undefined {
...
function isExpectedIncrement(
	...
	if (Node.isPostfixUnaryExpression(incrementor)) {
		...
		if (!Node.isIdentifier(operand) || operand.getText() !== variable)
			return false;
