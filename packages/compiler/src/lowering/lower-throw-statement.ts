...
export function lowerThrowStatement(
  ...
  if (!expr) {
    ...
    diagnostics.add({
      ...
      severity: "error",
      phase: "lowering",
      message: "`throw` without a value is not supported; throw the caught exception explicitly",
      span: {
  ...
  if (!Node.isNewExpression(expr)) {
    diagnostics.add({
      ...
      severity: "error",
      phase: "lowering",
      message: "throw must be `throw new <ErrorSubclass>(...)` or a re-throw of a caught exception",
      span: {
        filePath: statement.getSourceFile().getFilePath(),
        line: statement.getStartLineNumber(),
      },
  ...
  if (!Node.isIdentifier(callee)) {
    diagnostics.add({
      ...
      severity: "error",
      phase: "lowering",
      message: "throw target must be a simple class identifier (no qualified names yet)",
      span: {
        filePath: statement.getSourceFile().getFilePath(),
        line: statement.getStartLineNumber(),
      },
 ...
 *   ]
 */
function buildErrorHashMap(className: ErrorClassName, messageExpr: SqfExpression): SqfExpression {
  const hierarchy = ERROR_CLASS_HIERARCHIES[className];
  const hierarchyArray: SqfArrayExpression = {
    kind: "ArrayExpression",
    elements: hierarchy.map((c) => ({ kind: "Literal", text: JSON.stringify(c) })),
  };
