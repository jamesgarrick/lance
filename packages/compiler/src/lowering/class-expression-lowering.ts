...
export function tryLowerClassExpression(
	...
	if (Node.isNewExpression(expression)) {
		...
		const args = expression
			...
			.map((a) =>
				lowerExpression(a, diagnostics, bindings, semanticContext, scope),
			);
		return {
			kind: "CallExpression",
	...
	if (Node.isCallExpression(expression)) {
		...
		if (Node.isIdentifier(receiver)) {
			const staticCall = bindings.classStaticMethods.get(
				`${receiver.getText()}.${methodName}`,
			);
			if (staticCall) {
				return {
					kind: "CallExpression",
		...
		const method = bindings.classInstanceMethods.get(
	...
	if (
		Node.isBinaryExpression(expression) &&
		expression.getOperatorToken().getText() === "instanceof"
	) {
