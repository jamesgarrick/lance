...
import { Node, type CaseClause, type Expression, type Statement, type SwitchStatement } from "ts-morph";
import type { DiagnosticBag } from "../compiler/diagnostics";
import type { SqfStatement, SqfSwitchCase, SqfSwitchStatement } from "../ir/nodes";

export interface SwitchLoweringContext {
  readonly lowerExpression: (expr: Expression) => import("../ir/nodes").SqfExpression;
...
export function lowerSwitchStatement(
  ...
  for (const clause of clauses) {
    if (Node.isCaseClause(clause)) {
      ...
      const labels = [...pendingLabels.map(ctx.lowerExpression), ctx.lowerExpression(clause.getExpression())];
      pendingLabels = [];
      ...
      if (pendingLabels.length > 0) {
        diagnostics.add({
          ...
          severity: "error",
          phase: "lowering",
          message: "fallthrough into `default` is not supported; restructure as separate cases",
          span: {
      ...
      defaultCase = lowerDefaultBody(clause.getStatements(), ctx, diagnostics, clause);
    }
  }
