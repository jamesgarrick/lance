...
export function getPrimitiveWrapperBackingFieldName(
	...
	const firstPrivateField = cls
		.getMembers()
		.find(
			(m): m is PropertyDeclaration =>
				Node.isPropertyDeclaration(m) && m.hasModifier("private"),
		);
...
function isLanceCorePrimitiveInterfaceFile(
	filePath: string,
	lanceCorePackageName: string,
): boolean {
	// Handles both:
	...
	return (
		normalized.includes(`${pkg}/types/string`) ||
