...
function emitReturnStatement(statement: SqfReturnStatement): string {
	...
	const text = statement.expression
		? emitSqfExpression(statement.expression)
		: "";
	return `/* unsupported: return ${text} */`;
}

function emitTrailingExpressionStatement(
	statement: SqfTrailingExpressionStatement,
): string {
...
function emitIfStatement(statement: SqfIfStatement): string {
	...
	for (const s of statement.thenStatements)
		lines.push(`    ${emitSqfStatement(s)}`);
	if (hasElse) {
		lines.push("} else {");
		for (const s of statement.elseStatements)
			lines.push(`    ${emitSqfStatement(s)}`);
	}
	lines.push("};");
	return lines.join("\n");
...
function emitForFromToStatement(statement: SqfForFromToStatement): string {
	const stepClause = statement.step
		? ` step ${emitSqfExpression(statement.step)}`
		: "";
	return [
...
function emitSwitchStatement(statement: SqfSwitchStatement): string {
	const lines: string[] = [
		`switch (${emitSqfExpression(statement.discriminant)}) do {`,
	];
	...
	for (const branch of statement.cases) {
		...
		if (branch.body.length === 0) {
			...
			for (const stmt of branch.body)
				lines.push(`        ${emitSqfStatement(stmt)}`);
			lines.push("    };");
		}
	}
	...
	if (statement.defaultCase) {
		if (statement.defaultCase.length === 0) {
			...
			lines.push("    default {");
			for (const stmt of statement.defaultCase)
				lines.push(`        ${emitSqfStatement(stmt)}`);
			lines.push("    };");
		}
	}
...
function emitConditionalExpression(
	expression: SqfConditionalExpression,
): string {
