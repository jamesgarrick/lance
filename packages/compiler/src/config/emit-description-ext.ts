...
export function emitDescriptionExt(mission: MissionConfig): string {
  ...
  if (mission.debriefing?.length) sections.push(emitDebriefingClass(mission.debriefing));
  ...
  if (mission.function?.length) sections.push(emitFunctionsClass(mission.function));
...
function emitHeaderClass(m: MissionConfig): string | null {
  if (m.gameType === undefined && m.minPlayers === undefined && m.maxPlayers === undefined) {
    return null;
  }
...
function emitDebriefingClass(ends: DebriefingEnd[]): string {
  const inner = ends
    .map((e) => {
      ...
      if (e.pictureBackground !== undefined) str(lines, "pictureBackground", e.pictureBackground);
      if (e.soundBackground !== undefined) str(lines, "soundBackground", e.soundBackground);
    ...
    .map(([tag, fns]) => {
      const fnBlocks = fns
        .map((fn) => {
          ...
          if (fn.recompile !== undefined) bool(lines, "recompile", fn.recompile);
...
function strArr(lines: string[], key: string, value: string[] | undefined): void {
  if (value?.length) {
...
function numArr(lines: string[], key: string, value: number[] | undefined): void {
  if (value?.length) {
