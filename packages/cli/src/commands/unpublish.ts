...
import { defaultRegistry, readManifest, splitPackageAndVersion } from "../lib/manifest";
...
export default class Unpublish extends Command {
  ...
  async run(): Promise<void> {
    ...
    const manifest = await readManifest(process.cwd()).catch(() => ({ name: "", version: "", type: "library" as const }));
    const registry = defaultRegistry(manifest);
    await registryFetch(registry, `/packages/${encodeURIComponent(name)}/versions/${encodeURIComponent(version)}`, {
      method: "DELETE",
      token,
    });
