...
import { defaultRegistry, ensurePackageName, readManifest, splitPackageAndVersion, writeLockfile, writeManifest } from "../lib/manifest";
...
export default class Add extends Command {
  ...
  static override flags = {
    exact: Flags.boolean({ description: "Pin exact version instead of ^range", default: false }),
  };
  ...
  async run(): Promise<void> {
    ...
    const depRange = version
      ? (semver.valid(version) ? (flags.exact ? version : `^${version}`) : version)
      : (flags.exact ? resolved : `^${resolved}`);
    ...
    for (const [pkg, range] of Object.entries(manifest.dependencies)) {
      const pkgInfo = pkg === name ? info : await registryFetch<PackageInfoResponse>(registry, `/packages/${encodeURIComponent(pkg)}`);
      const v = pkgInfo.versions.filter((x) => semver.satisfies(x, range)).at(-1);
