...
export async function readManifest(cwd: string): Promise<LanceManifest> {
  ...
  if (!(await file.exists())) throw new Error(`${MANIFEST_FILE} not found in ${cwd}`);
  return JSON.parse(await file.text()) as LanceManifest;
}

export async function writeManifest(cwd: string, manifest: LanceManifest): Promise<void> {
  const path = join(cwd, MANIFEST_FILE);
...
export async function writeLockfile(cwd: string, lock: LanceLockfile): Promise<void> {
  const path = join(cwd, LOCK_FILE);
  await Bun.write(path, `${JSON.stringify(lock, null, 2)}\n`);
}

export function defaultRegistry(manifest: LanceManifest): string {
  return manifest.registry ?? process.env.LANCE_REGISTRY_URL ?? "http://localhost:8787";
}
...
export function splitPackageAndVersion(raw: string): { name: string; version: string | null } {
  const match = raw.match(/^(@[^/]+\/[^@]+)(?:@(.+))?$/);
  if (!match) throw new Error(`Invalid package spec: ${raw}. Expected @scope/name or @scope/name@version`);
